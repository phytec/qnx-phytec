...
/* Controller states */
#define USB_CONTROLLER_ACTIVE				0x1
#define USB_CONTROLLER_INACTIVE				0x0
#define USB_CONTROLLER_STATE_MASK			0x3
#define USB_CONTROLLER_STATE_POS			0x0
#define USB_CONTROLLER_GET_STATE(x)			((x & USB_CONTROLLER_STATE_MASK ) >> USB_CONTROLLER_STATE_POS)

#define USB_CONTROLLER_ERROR_STATE_MASK		0xF0
#define USB_CONTROLLER_ERROR_STATE_POS		0x04 /* usb_status_type_e */
#define USB_CONTROLLER_GET_ERROR_STATE(x)	((x & USB_CONTROLLER_ERROR_STATE_MASK ) >> USB_CONTROLLER_ERROR_STATE_POS)

typedef enum _usb_status_type {
    USB_BUS_STATUS_NA,				/* Unused, for compatibility with old stack */
    USB_BUS_STATUS_OK,
    USB_BUS_STATUS_ERROR,
    USB_BUS_STATUS_CRITICAL,
} usb_status_type_e;

typedef struct _usb_status {
	uint32_t              path;
	usb_status_type_e     type;
	uint32_t              value;
} usb_status_t;
...
typedef struct _usb_hcd_self {
	...
	int 				(*hc_report_bus_status)              ( usb_hcd_t *uhcd, usb_status_t *uhcerr );
...
#define IOUSB_EVENT_BUS_STATUS_UPDATE                   56
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/services/usb/stack/core/public/sys/io-usb-otg.h $ $Rev: 907221 $")
#endif
