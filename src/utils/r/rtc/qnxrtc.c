...
static rtc_alarm_cmd enable_alarm = RTC_ALARM_NONE;
...
#define NET_CLOCK_IDX 	0
	...
	{ "mx8xpsc",		init_mx8xpsc, get_mx8xpsc, set_mx8xpsc },
#endif
	{ "ultrazed",		init_ultrazed, get_ultrazed, set_ultrazed },
#endif
#ifdef	__ARM__
	...
	{ "mx7srtc",		init_mx7srtc, get_mx7srtc, set_mx7srtc},
	{ "ds1339c",		init_ds1339c, get_ds1339c, set_ds1339c },
	{ "rtc8564",		init_rtc8564, get_rtc8564, set_rtc8564 },
	{ "atsama5d3x",		init_atsama5d3x, get_atsama5d3x, set_atsama5d3x },
...
main(int argc, char *argv[]) {
	...
	chip.optstr = NULL;
	while ((opt=getopt(argc,argv,"b:ls" SLOWADJUST_OPTIONS RTCALARM_OPTIONS VERBOSE_OPTIONS))!=-1) {		switch(opt) {
		case 'b':
...
#ifdef RTCALARM
		case 'a':
			alarm_cmd = atoi(optarg);
			if (alarm_cmd) {
				enable_alarm = RTC_ALARM_ON;
			} else {
				enable_alarm = RTC_ALARM_OFF;
			}
			break;
	...
	if (set_alarm) {
		#ifdef VERBOSE_SUPPORTED
		if (verbose) {
			printf("rtc: setting rtc alarm to %d sec\n", alarm_time);
		}
		#endif

		if (rtc_alarm[alarm_idx].set_alarm(alarm_time) < 0) {
			exit(EXIT_FAILURE);
		}
	}
	...
	if (enable_alarm) {
		#ifdef VERBOSE_SUPPORTED
		if (verbose) {
			printf("rtc: %s rtc alarm\n", (enable_alarm == RTC_ALARM_OFF)?"disabling":"enabling");
		}
		#endif

		if (rtc_alarm[alarm_idx].enable_alarm(enable_alarm) < 0) {
			exit(EXIT_FAILURE);
		}
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/utils/r/rtc/qnxrtc.c $ $Rev: 891264 $")
#endif
