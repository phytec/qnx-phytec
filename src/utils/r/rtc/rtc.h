...
struct rtc_alarm_desc {
		const char	*name;
		int			(*set_alarm)(uint32_t);
		int			(*enable_alarm)(rtc_alarm_cmd);
};
#endif
...
#ifdef RTCALARM
extern int set_alarm_pmqcom(uint32_t time);
extern int enable_alarm_pmqcom(rtc_alarm_cmd cmd);
#endif
#endif
...
#if (defined VARIANT_imx8xp)
extern int init_mx8xpsc(struct chip_loc *chip, char *argv[]);
extern int get_mx8xpsc(struct tm *tm, int cent_reg);
extern int set_mx8xpsc(struct tm *tm, int cent_reg);
#endif

extern int init_ultrazed(struct chip_loc *chip, char *argv[]);
extern int get_ultrazed(struct tm *tm, int cent_reg);
extern int set_ultrazed(struct tm *tm, int cent_reg);
#endif
...
extern int init_mx7srtc(struct chip_loc *chip, char *argv[]);
extern int get_mx7srtc(struct tm *tm, int cent_reg);
extern int set_mx7srtc(struct tm *tm, int cent_reg);
...
extern int init_atsama5d3x(struct chip_loc *chip, char *argv[]);
extern int get_atsama5d3x(struct tm *tm, int cent_reg);
extern int set_atsama5d3x(struct tm *tm, int cent_reg);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/utils/r/rtc/rtc.h $ $Rev: 891264 $")
#endif
