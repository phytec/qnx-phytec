...
RTCFUNC(init,net)(struct chip_loc *chip, char *argv[]) {
	...
	ret = snprintf(buf, BUFFER_SIZE, "%sproc/1/as", buf2);
	if(ret < 0) {
		fprintf(stderr,"%s - Error encoding buffer path\n", __FUNCTION__);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/utils/r/rtc/nto/clk_net.c $ $Rev: 888596 $")
#endif
