...
int
mx6q_process_queue(void *arg, __attribute__((unused)) struct nw_work_thread *wtp)
{
    mx6q_dev_t      *mx6q = arg;
...
mx6q_isr(void *arg, __attribute__((unused)) int iid)
{
    mx6q_dev_t          *mx6q;
    ...
    if (mx6q->num_rx_queues <= MAX_NUM_QUEUES) {
        if(mx6q->num_rx_queues >= 2) {
...
//
const struct sigevent *
mx6q_isr_rx(void *arg, __attribute__((unused)) int iid)
{
    mx6q_dev_t          *mx6q;
    struct _iopkt_inter *ient;
    volatile uint32_t   *base;
...
const struct sigevent *
mx6q_1588_isr(void *arg, __attribute__((unused)) int iid)
{
    mx6q_dev_t          *mx6q;
    volatile uint32_t   *base;
    ...
    // read interrupt cause bits
    ievent = *(base + MX6Q_IEVENT);
    // Process only the 1 second rollover interrupt if it is active
    if ((ievent & IEVENT_TS_TIMER) != 0) {
        mx6q->rtc_half = 0;
        mx6q->rtc++;
        *(base + MX6Q_IEVENT) = IEVENT_TS_TIMER;
    ...
    // Process the interrupt that occurs halfway through each 1 second period
    mx6q->rtc_half = 1;
    InterruptUnlock(&mx6q->spinlock);
...
int
mx6q_enable_interrupt(__attribute__((unused)) void *arg)
{
    return 0;
}

int
mx6q_process_interrupt(void *arg, __attribute__((unused)) struct nw_work_thread *wtp)
{
    mx6q_dev_t      *mx6q = arg;
...
mx6q_process_interrupt_rx(void *arg, struct nw_work_thread *wtp)
{
    ...
    for (;;) {
        ...
        *(base + MX6Q_IEVENT) = ievent;
        ...
        if (mx6q->cfg.verbose > 6) {
            log(LOG_ERR, "%s(): ievent 0x%x", __FUNCTION__, ievent);
        }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devnp/mx6x/event.c $ $Rev: 914475 $")
#endif
