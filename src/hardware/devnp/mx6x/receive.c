...
mx6q_receive (mx6q_dev_t *mx6q, struct nw_work_thread *wtp, uint8_t queue)
{
    ...
    for(;;) {
        if (mx6q->cfg.verbose > 5) {
            slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s(): rx_cidx %d queue %d",
	    ...
	    if (status & RXBD_ERR) {
	        ...
	        mx6q_add_pkt(mx6q, mx6q->rx_pkts[offset + this_idx],
			     offset + this_idx);
	        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s(): status RXBD_ERR 0x%X", __FUNCTION__, status);
	        ifp->if_ierrors++;
	    ...
	    if (new == NULL) {
	        ...
	        mx6q_add_pkt(mx6q, mx6q->rx_pkts[offset + this_idx],
			     offset + this_idx);
	        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s(): mbuf alloc failed!", __FUNCTION__);
	    ...
	    if (mx6q->cfg.verbose > 7) {
	        ...
	        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR,"Rxd dev_idx %d bytes %d\n",
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devnp/mx6x/receive.c $ $Rev: 910691 $")
#endif
