...
static int mx6q_tx (mx6q_dev_t *mx6q, struct mbuf *m, uint8_t queue)
{
    struct mbuf		*m2;
	...
	*(base + MX6Q_R_SECTION_EMPTY_ADDR) = 0;
    ...
    switch(mx6q->num_tx_queues) {
        ...
        case(1):
            ...
            break;
        default:
            log(LOG_ERR, "%s():Invalid number of Tx Buffers", __FUNCTION__);
            return EINVAL;
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devnp/mx6x/transmit.c $ $Rev: 891479 $")
#endif
