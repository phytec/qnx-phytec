...
int mx6q_tx (mx6q_dev_t *mx6q, struct mbuf *m, uint8_t queue)
{
	...
	if ((m2 = mx6q_defrag(m)) == NULL) {
	    slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s(): mx6q_defrag() failed", __FUNCTION__);
	...
	slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR,"Txd dev_idx %d  num_frag %d  first descr bytes %d",
	...
	if (tx_bd->status & TXBD_R) {
	    // this should NEVER happen
	    slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s(): Tx descriptors hit end",
		__FUNCTION__);
	...
	if (mx6q->cfg.verbose > 6) {
	    slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR,"normal: idx %d len %d stat 0x%X",
	...
	if (mx6q->cfg.verbose > 6) {
	    slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR,"enable: single: stat 0x%X", tx_bd->status);
	}
	...
	if (mx6q->cfg.verbose > 6) {
	    slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR,"enable: multi: first stat 0x%X last stat 0x%X",
	...
	default:
	    InterruptUnlock(&mx6q->spinlock);
	    slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "Bad queue %d", queue);
	    break;
	}
    }
    return EOK;
...
mx6q_transmit_complete(mx6q_dev_t *mx6q, uint8_t queue)
{
	...
	if (mx6q->cfg.verbose > 6) {
	    slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR,"tx done: idx %d status 0x%x estatus 0x%x bdu 0x%x",
    ...
    if (mx6q->cfg.verbose > 5) {
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s(): ending: tx_pidx %d tx_cidx %d tx_descr_inuse %d",
        ...
        struct sockaddr *dst, struct rtentry *rt)
{
    ...
    /* To prevent above array bounds compiler warning */
    if (queue >= MAX_NUM_QUEUES) {
        panic("%s - Invalid queue index (%d) request", __FUNCTION__, queue);
        return EINVAL;
    }
	...
	*(base + MX6Q_R_SECTION_EMPTY_ADDR) = 0;
    ...
    switch(mx6q->num_tx_queues) {
        ...
        case(1):
            ...
            break;
        default:
            slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s():Invalid number of Tx Buffers", __FUNCTION__);
            return EINVAL;
    }
    ...
    if (mx6q->cfg.verbose) {
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s: q1_bw %u q2_bw %u q1_idle %u q2_idle %u",
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devnp/mx6x/transmit.c $ $Rev: 910691 $")
#endif
