...
void mx6q_timer_init (mx6q_dev_t *mx6q)
{
    ...
    *(tbase + GPT_CR) = GPT_CR_SWR | GPT_CR_CLKSRC_PERI | GPT_CR_STOPEN |
    ...
    intid = InterruptAttachEvent(mx6q->tirq, &event, 0);
    tid = pthread_self();
}
...
int mx6q_timer_delay (mx6q_dev_t *mx6q, struct ifdrv *ifd)
{
    ...
    if (ISSTACK) {
	slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "mx6x precise delay called from stack context");
	return EWOULDBLOCK;
    }
    ...
    *(tbase + GPT_CR) = GPT_CR_SWR | GPT_CR_CLKSRC_PERI | GPT_CR_STOPEN |
      GPT_CR_DOZEEN | GPT_CR_WAITEN | GPT_CR_DBGEN | GPT_CR_ENMOD;
    InterruptUnmask(mx6q->tirq, intid);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devnp/mx6x/timer.c $ $Rev: 910691 $")
#endif
