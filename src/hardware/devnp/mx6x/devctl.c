...
mx6q_ioctl(struct ifnet *ifp, unsigned long cmd, caddr_t data)
{
	...
	case SIOCGDRVSPEC:
		...
		case ENABLE_BRCM_PHY_LOWPOWER:
			error = mx6q_br_lowpower_enable_ioctl(mx6q);
			break;

		case DISABLE_BRCM_PHY_LOWPOWER:
			error = mx6q_br_lowpower_disable_ioctl(mx6q);
			break;
...
int mx6q_br_lowpower_enable_ioctl(mx6q_dev_t *mx6q)
{
	nic_config_t	*cfg = &mx6q->cfg;
	int				phy_idx = cfg->phy_addr;
	uint16_t		val = 0;
...
int mx6q_br_lowpower_disable_ioctl(mx6q_dev_t *mx6q)
{
	nic_config_t	*cfg = &mx6q->cfg;
	int				phy_idx = cfg->phy_addr;
	uint16_t		val = 0;
    ...
    while (length < mii_op_size) {
        ...
        switch(mii_op.op) {
            ...
            case WRITE:
                ...
                mii_op.err = 0;
				if (cfg->verbose > 5) {
					log(LOG_INFO, "%s(): op WRITE:  %x,%x,%x\n", __FUNCTION__, mii_op.addr, mii_op.reg, mii_op.data);
				}
                break;
            default:
                log(LOG_ERR, "%s(): unkownn mii_op  %d\n", __FUNCTION__, mii_op.op);
                mii_op.err = -1;
                err = EINVAL;
        }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devnp/mx6x/devctl.c $ $Rev: 895010 $")
#endif
