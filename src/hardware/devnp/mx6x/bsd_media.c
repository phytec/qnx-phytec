...
bsd_mii_mediastatus (struct ifnet *ifp, struct ifmediareq *ifmr)
{
	...
	default:
	slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "mx6q: Unknown media, forcing none");
	/* Fallthrough */
	case 0:
	    mx6q->bsd_mii.mii_media_active |= IFM_NONE;
	    break;
...
bsd_mii_mediachange (struct ifnet *ifp)
{
	...
	slog2f(NULL, _SLOGC_NETWORK, _SLOG_WARNING,
    ...
    #ifdef MX6UL
    /* MDI_PowerdownPhy will reset all settings
     * made in mx6_ksz8081_phy_init(). Need to call mx6_ksz8081_phy_init()
     * again to set the registers after MDI_PowerupPhy.
     */
    if ( (mx6q->mdi->PhyData[cfg->phy_addr]->VendorOUI == KENDIN) &&
         (mx6q->mdi->PhyData[cfg->phy_addr]->Model == KSZ8081) )
    {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devnp/mx6x/bsd_media.c $ $Rev: 910691 $")
#endif
