...
mx6q_parse_options (mx6q_dev_t *mx6q, char *options, nic_config_t *cfg, struct mx6q_arg *mx6q_arg)
{
    ...
    if (hwi_off_gpt != HWI_NULL_OFF) {
        ...
        if (tag_base && tag_irq) {
            ...
            if (cfg->verbose) {
                log(LOG_INFO, "GP Timer base address = 0x%08X", (uint32_t) tbase);
            }
            /* GPT IRQ */
            tirq = hwitag_find_ivec(hwi_off_gpt, NULL);
            if (cfg->verbose) {
                log(LOG_INFO, "GP Timer Interrupt = %d", tirq);
            }
            ...
            if(tbase && tirq) {
                ...
                log(LOG_ERR, "%s(): Invalid GPT information - Setting GPT Base and GPT IRQ to default", __FUNCTION__);
            }
        } else {
            log(LOG_ERR, "%s(): Missing GPT information - Setting GPT Base and GPT IRQ to default", __FUNCTION__);
        }
    }
    ...
    while (options && *options != '\0') {
        ...
        case 16:
            ...
            if (mx6q->cfg.verbose) {
                log(LOG_INFO, "PHY Addr after hwitag checked: 0x%x",
                    mx6q->cfg.phy_addr);
            }
            ...
            if (mx6q->cfg.verbose) {
                log(LOG_INFO, "PHY Addr from command line: 0x%x",
                    mx6q->cfg.phy_addr);
            }
            break;
        default:
    ...
    if (mx6q->cfg.verbose) {
        log(LOG_INFO, "PHY Addr (or Device Index): 0x%x IRQ_0: %d  IRQ_1: %d",
            mx6q->cfg.phy_addr,
            cfg->irq[0],
            cfg->irq[1]);
    }
...
static void *mx6q_rx_thread (void *arg)
{
    mx6q_dev_t        *mx6q = arg;
    int                rc, queue;
    struct _pulse      pulse;
...
static void mx6q_rx_thread_quiesce (void *arg, int die)
{
    mx6q_dev_t          *mx6q = arg;
...
static int
mx6q_attach(__attribute__((unused)) struct device *parent, struct device *self, void *aux)
{
    mx6q_dev_t         *mx6q;
    nic_config_t       *cfg;
    int                 rc;
    uint32_t            queue, offset;
    int                 i;
    struct ifnet       *ifp;
    size_t              size;
    char               *options;
...
#ifdef MX8XP
    if (mx6q->fd == NOFD) {
        slogf(_SLOGC_NETWORK, _SLOG_ERROR, "Attach Error: pkt_typed_mem_fd is NOFD");
        return ENOMEM;
    }
#endif
    ...
    if (cfg->verbose) {
        log(LOG_INFO, "Device Index: %d Lan Index: %d",
            cfg->device_index, cfg->lan);

        log(LOG_INFO, "Register Base:0x%"PRIx64"  GP Timer Base:0x%"PRIx64" ",
    ...
    if (mx6q->num_rx_descriptors < MIN_NUM_RX_DESCRIPTORS) {
        ...
        log(LOG_WARNING, "%s: Requested Rx Descriptors too low. Setting to : %d", __FUNCTION__, MIN_NUM_RX_DESCRIPTORS);
    }
    if (mx6q->num_rx_descriptors > MAX_NUM_RX_DESCRIPTORS) {
        mx6q->num_rx_descriptors = MAX_NUM_RX_DESCRIPTORS;
        log(LOG_WARNING, "%s: Requested Rx Descriptors too high. Setting to : %d", __FUNCTION__, MAX_NUM_RX_DESCRIPTORS);
    }

    mx6q->num_tx_descriptors &= ~3;
    if (mx6q->num_tx_descriptors < MIN_NUM_TX_DESCRIPTORS) {
        mx6q->num_tx_descriptors = MIN_NUM_TX_DESCRIPTORS;
        log(LOG_WARNING, "%s: Requested Tx Descriptors too low. Setting to : %d", __FUNCTION__, MIN_NUM_TX_DESCRIPTORS);
    }
    if (mx6q->num_tx_descriptors > MAX_NUM_TX_DESCRIPTORS) {
        mx6q->num_tx_descriptors = MAX_NUM_TX_DESCRIPTORS;
        log(LOG_WARNING, "%s: Requested Tx Descriptors too high. Setting to : %d", __FUNCTION__, MAX_NUM_TX_DESCRIPTORS);
    }
    ...
    if (mx6q->cfg.verbose > 5) {
        slogf(_SLOGC_NETWORK, _SLOG_INFO,
            "%s:%d - Register SMMU map tx decriptor area: Address:0x%lx",
            __FUNCTION__, __LINE__, mx6q->tx_descriptor_area_phys);
    }
...
#if defined MX8XP
    // smmuman needs the physical address of the rx descriptors
    mx6q->rx_descriptor_area_phys = vtophys(mx6q->rx_bd);

    if (mx6q->cfg.verbose > 5) {
        slogf(_SLOGC_NETWORK, _SLOG_INFO,
            "%s:%d - Register SMMU map rx decriptor area: Address:0x%lx",
            __FUNCTION__, __LINE__, mx6q->rx_descriptor_area_phys);
    }
    ...
    switch(mx6q->num_tx_queues) {
        ...
        case(1):
            *(base + MX6Q_X_DES_START) = vtophys((void *)&mx6q->tx_bd[0]);
            break;
        default:
            log(LOG_ERR, "%s():Invalid number of Tx Queues", __FUNCTION__);
            return EINVAL;
    }
    switch(mx6q->num_rx_queues) {
        ...
        case(1):
            *(base + MX6Q_R_DES_START) = vtophys((void *)&mx6q->rx_bd[0]);
            break;
        default:
            log(LOG_ERR, "%s():Invalid number of Rx Queues", __FUNCTION__);
            return EINVAL;
    }
    ...
    *(base + MX6Q_IEVENT) = 0xffffffff;
    ...
    /* Attach to hardware interrupts
     *
     * On the S32V boards both the IEVENT_TS_TIMER and MX6Q_TCSR0 interrupt
     * will be sent to IRQ[0]
     */

    if (mx6q->iid_1588 == -1) {
        if ((rc = InterruptAttach_r(mx6q->cfg.irq[0], mx6q_1588_isr,
            mx6q, sizeof(*mx6q), _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
            rc = -rc;
            log(LOG_ERR, "%s(): InterruptAttach_r(1588) failed: %d", __FUNCTION__, rc);
            mx6q_destroy(mx6q, 12);
    ...
    *(base + MX6SLX_RXIC0) = rxic_val;
    ...
    if (mx6q->iout < 0 || mx6q->iout > 3) {
        slogf (_SLOGC_NETWORK, _SLOG_INFO, "Invalid iout value %d - using default", mx6q->iout);
        mx6q->iout = 2;
    }
    if (mx6q->slew < 0 || mx6q->slew > 3) {
        slogf (_SLOGC_NETWORK, _SLOG_INFO, "Invalid slew value %d - using default", mx6q->slew);
        mx6q->slew = 2;
    }
    // Do one time PHY initialization
    if (mx6q_init_phy(mx6q)) {
        log(LOG_INFO, "fec%d: PHY init failed", cfg->lan);

        /* release all the allocated resources */
        mx6q_destroy(mx6q, 13);

        if (cfg->verbose) {
            log(LOG_INFO, "fec%d: PHY Interface Destroyed: how=13", cfg->lan);
        }
        return ENODEV;
    }
    ...
    mx6q_ptp_start(mx6q);
    if (cfg->verbose) {
        log(LOG_INFO, "Configured PTP 1588 timer: fec%d", cfg->lan);
    }
    ...
    if (cfg->verbose > 3) {
        log(LOG_INFO, "successfully attached fec%d\n", cfg->lan);
    }
...
mx6q_destroy(mx6q_dev_t *mx6q, int how)
{
    ...
    case 7:
        ...
        /* UNmap the Rx Descriptor resources */
        rc = smmu_unmap_driver_memory(mx6q->rx_descriptor_area_phys,
                                      MX6Q_MAP_SIZE);
        if (rc) {
            slogf(_SLOGC_NETWORK, _SLOG_ERROR,
                "%s:%d - SMMUMAN 7 retcode: %d",
                __FUNCTION__, __LINE__, rc);
        }
#endif
    ...
    case 5:
        ...
        /* UNmap the Tx Descriptor resources */
        rc = smmu_unmap_driver_memory(mx6q->tx_descriptor_area_phys,
                                      MX6Q_MAP_SIZE);
        if (rc) {
            slogf(_SLOGC_NETWORK, _SLOG_ERROR,
                "%s:%d - SMMUMAN 5 retcode: %d",
                __FUNCTION__, __LINE__, rc);
        }
#endif
    ...
    case 3:
        munmap_device_memory((uint32_t*)mx6q->reg, MX6Q_MAP_SIZE);

        if(mx6q->phy_reg && (mx6q->phy_reg != mx6q->reg)) {
            munmap_device_memory((uint32_t*)mx6q->phy_reg, MX6Q_MAP_SIZE);
        }
        /* fallthrough */
...
static int
mx6q_detach(struct device *dev, __attribute__((unused)) int flags)
{
    mx6q_dev_t  *mx6q;
...
#ifdef IMX_DEVNP_DEBUG
static void DumpPhy(mx6q_dev_t *mx6q)
{
    uint32_t      i;
    nic_config_t *cfg     = &mx6q->cfg;
        ...
        switch(mx6q->num_rx_queues) {
            ...
            case(1):
                *(base + MX6Q_R_DES_ACTIVE) = R_DES_ACTIVE;
                break;
            default:
                log(LOG_ERR, "%s():Invalid number of Rx Buffers", __FUNCTION__);
                return EINVAL;
        }
    }
...
static void
mx6q_stop(struct ifnet *ifp, __attribute__((unused)) int disable)
{
    mx6q_dev_t		*mx6q = ifp->if_softc;
    uint32_t		rx_active, tx_active, queue, offset;
    int             i;
...
int
mx6q_detect(void *dll_hdl, __attribute__((unused)) struct _iopkt_self *iopkt, char *options)
{
    struct mx6q_arg	mx6q_arg;
    struct device	*dev;
    ...
    IFNET_FOREACH(ifp) {
        ...
        if ((err == EOK) && (dcon->dcom_config.num_mem_windows > 0)   &&
            ...
            log(LOG_INFO, "ENET_1 Driver already loaded for %s",
                ifp->if_xname);
        ...
        if ((err == EOK) && (dcon->dcom_config.num_mem_windows > 0)   &&
            ...
            log(LOG_INFO, "ENET_2 Driver already loaded for %s",
                ifp->if_xname);
        ...
        if(enet1_is_configured && enet2_is_configured) {
            log(LOG_ERR, "Config Failed: Both Enet Interfaces already exist");
    ...
    for (i = 0; i < dev_num; i++) {
        ...
        if (enet1_is_configured && (mx6q_arg.idx == 0)) {
            log(LOG_ERR, "Adding Dev Index: %d It is already configured",
                mx6q_arg.idx);
        ...
        if (dev_attach("fec", options, &mx6q_ca, &mx6q_arg, &single,
            ...
            if (mx6q_arg.cfg.verbose > 3) {
                log(LOG_INFO, "device_attach(%d) FAILed\n", i);
            }
        } else {
            /* at least 1 interface has been added to io-pkt */
            num_interfaces_added++;
            if (mx6q_arg.cfg.verbose > 3) {
                log(LOG_INFO, "attached index:%d of %d device(s): Config idx:%d",
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devnp/mx6x/detect.c $ $Rev: 914475 $")
#endif
