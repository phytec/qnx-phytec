...
#define MAX_TIME_DIFF       999990000
#define MIN_TIME_DIFF       10000
...
void mx6q_ptp_cal (mx6q_dev_t *mx6q)
{
    ...
    *(base + MX6Q_TIMER_CTRLR) = MX6Q_TIMER_CTRL_CAPTURE;
    ...
    if (timo <= 0) {
	slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "Timestamp capture failed");
    }
	...
	slog2f(NULL,_SLOGC_NETWORK, _SLOG_INFO, "Timestamp clock %dMHz", 1000 / mx6q_clock_period);
    } else {
	slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "Count %d in %d ns so timestamp clock frequency unknown, defaulting to 125MHz", count, diff);
	mx6q_clock_period = 8;
    }
...
void mx6q_ptp_start (mx6q_dev_t *mx6q)
{
    ...
    } else if (CLOCK_FREQ > 0) {
        mx6q_clock_freq = CLOCK_FREQ;
        mx6q_clock_period = (1000 * 1000 * 1000) / mx6q_clock_freq;
    } else {
        mx6q_ptp_cal(mx6q);
    }

    memset ((char *) tx_ts, 0, sizeof (tx_ts));
    memset ((char *) rx_ts, 0, sizeof (rx_ts));
    tx_ts_cnt = rx_ts_cnt = 0;
    ...
    *(base + MX6Q_TIMER_CTRLR) = MX6Q_TIMER_CTRL_PEREN;
    ...
    if (mx6q->cfg.verbose > 4) {
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_INFO, "1588 Clock configuration:");
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_INFO, "\t..Clock frequency=%u (Hz)", mx6q_clock_freq);
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_INFO, "\t..Clock period=%u (ns. per tick)", mx6q_clock_period);
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_INFO, "\t..Event period=%u (ns.)", MX6Q_TIMER_PER1SEC);
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_INFO, "Enable 1588 clock counter");
    }
    // Enable timer
    *(base + MX6Q_TIMER_CTRLR) |= MX6Q_TIMER_CTRL_EN;
...
int mx6q_ptp_is_eventmsg (struct mbuf *m, ptpv2hdr_t **ph)
{
    ...
    if (m == NULL) {
	slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "%s: NULL mbuf", __FUNCTION__);
	return 0;
    }
...
void mx6q_ptp_add_rx_timestamp (mx6q_dev_t *mx6q, ptpv2hdr_t *ph, mpc_bd_t *bd)
{
     ...
     */
    if ((rx_ts[rx_ts_cnt].ts.nsec > MAX_TIME_DIFF) &&
	(mx6q->rtc_half == 0)) {
	/*
    ...
    } else if ((rx_ts[rx_ts_cnt].ts.nsec < MIN_TIME_DIFF) &&
	       (mx6q->rtc_half == 1)) {
	/*
    ...
    if (mx6q->cfg.verbose > 4) {
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "RX 1588 message:");
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "\t..message = %u", (ph->messageId & 0x0f));
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "\t..sequence = %u", ntohs(ph->sequenceId));
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "\t..timestamp = %u.%u\n", mx6q->rtc, bd->timestamp);
    }
}
...
void mx6q_ptp_add_tx_timestamp (mx6q_dev_t *mx6q, ptpv2hdr_t *ph, mpc_bd_t *bd)
{
     ...
     * seconds stored in software.
     */
    if ((tx_ts[tx_ts_cnt].ts.nsec > MAX_TIME_DIFF) &&
	(mx6q->rtc_half == 0)) {
	/*
	 * Captured from hardware before the second but by the time
	 * we read the software seconds then it had incremented.
    ...
    } else if ((tx_ts[tx_ts_cnt].ts.nsec < MIN_TIME_DIFF) &&
	       (mx6q->rtc_half == 1)) {
	/*
	 * Captured from hardware after the second but the software
	 * seconds have not incremented yet.
    ...
    if (mx6q->cfg.verbose > 4) {
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "TX 1588 message:");
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "\t..message = %u", (ph->messageId & 0x0f));
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "\t..sequence = %u", ntohs(ph->sequenceId));
        slog2f(NULL,_SLOGC_NETWORK, _SLOG_ERROR, "\t..timestamp = %u.%u\n", mx6q->rtc, bd->timestamp);
    }
...
int mx6q_ptp_get_tx_timestamp (mx6q_dev_t *mx6q, ptp_extts_t *ts)
{
    ...
    for (i = 0; i < mx6q->num_tx_queues; i++) {
...
void mx6q_ptp_get_cnt (mx6q_dev_t *mx6q, ptp_time_t *cnt)
{
    ...
    *(base + MX6Q_TIMER_CTRLR) = tmp;
	 ...
	 * seconds stored in software.
	 */
	if ((cnt->nsec > MAX_TIME_DIFF) && (mx6q->rtc_half == 0)) {
	    /*
	     * Captured from hardware before the second but by the time
	     * we read the software seconds then it had incremented.
	     */
	    cnt->sec--;
	} else if ((cnt->nsec < MIN_TIME_DIFF) && (mx6q->rtc_half == 1)) {
	    /*
	     * Captured from hardware after the second but the software
	     * seconds have not incremented yet.
	     */
...
void mx6q_ptp_set_cnt (mx6q_dev_t *mx6q, ptp_time_t cnt)
{
    ...
    *(base + MX6Q_TIMER_VALUER) = cnt.nsec;
    ...
    /* Clear the Tx and Rx timestamp buffers */
    memset ((char *) tx_ts, 0, sizeof (tx_ts));
    memset ((char *) rx_ts, 0, sizeof (rx_ts));
    tx_ts_cnt = rx_ts_cnt = 0;
}
...

{
     ...
     * Try to find the corr_inc  between 1 to ptc.comp to
     * meet adjustment requirement.
     */
    lhs = NSEC_PER_SEC;
    rhs = (uint64_t)ptc.comp * (uint64_t)mx6q_clock_period;
    for (i = 1; i <= mx6q_clock_period; i++) {
        if (lhs >= rhs) {
            corr_inc = i;
            corr_period = lhs / rhs;
            break;
     ...
     */
    if (i > mx6q_clock_period) {
        corr_inc = mx6q_clock_period;
        corr_period = 1;
    }

    if (!ptc.positive)
        corr_ns = mx6q_clock_period - corr_inc;
    else
        corr_ns = mx6q_clock_period + corr_inc;

    InterruptLock(&mx6q->spinlock);

    tmp = *(base + MX6Q_TIMER_INCR) & MX6Q_TIMER_INCR_MASK;
    tmp |= corr_ns << MX6Q_TIMER_INCR_CORR_OFF;
    *(base + MX6Q_TIMER_INCR) = tmp;
    *(base + MX6Q_TIMER_CORR) = corr_period;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devnp/mx6x/ptp.c $ $Rev: 910691 $")
#endif
