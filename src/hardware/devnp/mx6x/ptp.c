...
static void mx6q_ptp_cal (mx6q_dev_t *mx6q)
{
...
void mx6q_ptp_start (mx6q_dev_t *mx6q)
{
    ...
    *(base + MX6Q_TIMER_CTRLR) = MX6Q_TIMER_CTRL_PEREN;
    ...
    if (mx6q->cfg.verbose > 4) {
        log(LOG_INFO, "1588 Clock configuration:");
        log(LOG_INFO, "\t..Clock frequency=%u (Hz)", mx6q_clock_freq);
        log(LOG_INFO, "\t..Clock period=%u (ns. per tick)", mx6q_clock_period);
        log(LOG_INFO, "\t..Event period=%u (ns.)", MX6Q_TIMER_PER1SEC);
        log(LOG_INFO, "Enable 1588 clock counter");
    }
    // Enable timer
    *(base + MX6Q_TIMER_CTRLR) |= MX6Q_TIMER_CTRL_EN;
...
int mx6q_ptp_is_eventmsg (struct mbuf *m, ptpv2hdr_t **ph)
{
    ...
    if (m->m_pkthdr.len <
	(int)(sizeof(struct ether_header) + sizeof(ptpv2hdr_t))) {
	return 0;
    }
	...
	if ((m->m_len - remain) < (int)sizeof(ptpv2hdr_t)) {
	    return 0;
	}
	if (ph != NULL) {
	...
	if (pktlen < (int)(sizeof(struct ether_header) + (iph->ip_hl << 2) +
 ...
 *
 */
int mx6q_ptp_get_rx_timestamp (ptp_extts_t *ts)
{
    int i;
...
void mx6q_ptp_get_cnt (mx6q_dev_t *mx6q, ptp_time_t *cnt)
{
	...
	InterruptUnlock(&mx6q->spinlock);
	if (timo <= 0) {
		slogf(_SLOGC_NETWORK, _SLOG_ERROR, "Timestamp capture failed");
	}
    }
}
...
int mx6q_ptp_ioctl (mx6q_dev_t *mx6q, struct ifdrv *ifd)
{
    ...
    if (ifd != NULL) {
		...
		found = mx6q_ptp_get_rx_timestamp(&ts);
	    } else {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devnp/mx6x/ptp.c $ $Rev: 911909 $")
#endif
