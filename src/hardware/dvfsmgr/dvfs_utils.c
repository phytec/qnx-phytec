	...
	while ((i = getopt(argc, argv, "vwp:i:c:d:u:U:sD:o:")) != -1) {
		switch ( i ) {
			...
			case 'i':
				...
				break;
			case 'c':
				strlcpy(dev->cfg_file_path, optarg, PATH_MAX);
				break;
			case 'o':
				strlcpy(dev->cfg_pwr_table_path, optarg, PATH_MAX);
				break;
			case 'w':
 ...
 * - fmt: Message
 */
void dvfslog(int severity, const char *func, int line, __attribute__((unused)) int verbose_level,
            const char *fmt, ...)
{
	va_list arglist;
	char buf[512];
 ...
 */
static int dvfs_check_power_levels(__attribute__((unused)) dvfs_t *dev, dvfs_table_t t[], int len)
{
	int ret = EFAULT;
 ...
 */
static int get_next_level_from_file(dvfs_t *dev, int fd, dvfs_table_t *lvl_desc, int *err)
{
	typedef enum {
 ...
 *
 */
static int dvfs_get_next_pwrtbl_row(dvfs_t *dev, int fd, dvfs_pwrtbl_t *lvl_desc, int *err)
 {
	typedef enum {
		COLUMN_OPP = 0,
		COLUMN_PLL_FREQ,
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/dvfsmgr/dvfs_utils.c $ $Rev: 894097 $")
#endif
