...
static int mx8x_get_temperature(dvfs_t *dev, char *corename, sc_rsrc_t resource)
{
	int						cnt;
	int						ret;
	sc_err_t				status;
 ...
 *  - -1 in case of error.
 */
int dvfs_thread_cpu_act(cpustate_t *cpustate)
{
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/dvfsmgr/aarch64/imx8x.le/dvfs_core.c $ $Rev: 888398 $")
#endif
