 ...
 */
static int dvfs_ctrl(dvfs_t *dev)
{
 ...
 */
static void dvfs_therm_thread(dvfs_t *dev)
{
	int temp;
	char thread_name[_NTO_THREAD_NAME_MAX];
 ...
 * - dev: A pointer to the global dvfs structure
 */
static void dvfs_bg_thread(dvfs_t *dev)
{
	int fd;
 ...
 */
static int remove_rapps(dvfs_t *dev)
{
	int i;
	int ret = EOK;
	related_apps_t *p = NULL;
...
static int dvfs_init_cpustate(dvfs_t *dev)
{
 ...
 */
static int dvfs_create_background_thread(dvfs_t *dev)
{
 ...
 * - Status of thread creation. EOK on success, EFAULT on failure
 */
static int dvfs_create_ctrl_thread(dvfs_t *dev)
{
	pthread_attr_t		pattr;
	struct sched_param	param;
 ...
 */
static int dvfs_create_therm_thread(dvfs_t *dev)
{
	pthread_attr_t		pattr;
	struct sched_param	param;
 ...
 */
static int dvfs_io_read(resmgr_context_t *ctp, io_read_t *msg, RESMGR_OCB_T *ocb)
{
	uint32_t        nreply;
	int             status;
 ...
 *
 */
static int dvfs_io_devctl(resmgr_context_t *ctp, io_devctl_t *msg, RESMGR_OCB_T *ocb)
{
 ...
 */
static void dvfs_shutdown(int signo)
{
	switch (signo) {
		case SIGHUP:
		case SIGINT:
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/dvfsmgr/dvfs.c $ $Rev: 894097 $")
#endif
