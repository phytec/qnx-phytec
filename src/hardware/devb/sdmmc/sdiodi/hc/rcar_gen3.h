...
#define RCAR_TUNING_DONE     2
    ...
    int             clkrate_offset;
    int             hs400_use_4tap;
    uint32_t        hs400_sdckcr;
    uint32_t        tap_hs400;

    /* WA for Errata No. 109 - adjust eMMC HS400 offset */
    ...
    /* WA for Errata No. 123 (step1) - SCC HS400 correction of sampling clock position */
...
static inline uint32_t sdmmc_tmpport_read(uintptr_t base, uint32_t addr)
{
    /* read mode */
...
static inline void sdmmc_tmpport_write(uintptr_t base, uint32_t addr, uint32_t val)
{
    /* write mode */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devb/sdmmc/sdiodi/hc/rcar_gen3.h $ $Rev: 908306 $")
#endif
