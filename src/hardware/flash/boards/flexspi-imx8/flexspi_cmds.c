 ...
 *
 * @param[in]  dev      File descriptor.
 * @param[out] stat_reg Memory status register.
 *
 * @return EOK always.
 */
static int read_flag_status(imx_fspi_t *dev, uint8_t *stat_reg)
{
    imx_flexspi_clear_fifo(dev, both);                                            /* Clear Rx Tx FIFOs */
    imx_flexspi_set_rx_watermark(dev, sizeof(*stat_reg));                         /* Set watermark*/
    imx_flexspi_send_ip_cmd(dev, IMX_FSPI_LUT_CMD_IDX_RFLAGR, sizeof(*stat_reg)); /* Send command */
 ...
 */
static int read_erase_status(imx_fspi_t *dev)
{
    uint8_t flag_reg = 0xFF;

    read_flag_status(dev, &flag_reg);
 ...
 */
int pre_ident_cfg(__attribute__((unused)) imx_fspi_t *dev)
{
    return EOK;
}
 ...
 */
int post_ident_cfg(__attribute__((unused)) imx_fspi_t *dev)
{
    return EOK;
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/flash/boards/flexspi-imx8/flexspi_cmds.c $ $Rev: 893539 $")
#endif
