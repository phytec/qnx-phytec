		...
		switch (c) {
			...
			case 'd':
				...
				do {
					...
					++optind;
					dev->funcs  = (spi_slave_funcs_t *)drventry;
					dev->devnum = devnum++;
					dev->dlhdl  = dlhdl;
...
start_spi:
	if (head) {
		...
		if (procmgr_daemon(0, PROCMGR_DAEMON_NOCLOSE | PROCMGR_DAEMON_NODEVNULL) == -1) {
			goto cleanup;
		}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/spi/slave/_spi_main.c $ $Rev: 855721 $")
#endif
