...
_spi_devctl(resmgr_context_t *ctp, io_devctl_t *msg, spi_ocb_t *ocb)
{
	...
	switch (msg->i.dcmd) {
        ...
        case DCMD_SPI_GET_ERROR:
        {
            int   *err;

            if (dev->funcs->error == NULL) {
                return ENOTSUP;
            }

            err = (int *)_DEVCTL_DATA(msg->o);

            status = dev->funcs->error(drvhdl, err);
            if (status != EOK) {
                return EINVAL;
            }

            msg->o.nbytes = sizeof(int);
            return _RESMGR_PTR(ctp, msg, sizeof(msg->o) + sizeof(int));
        }
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/spi/slave/_spi_devctl.c $ $Rev: 853252 $")
#endif
