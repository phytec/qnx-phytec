 ...
 */
static int32_t imx_capabilities(HW_CONTEXT_T *imx, ado_pcm_t *pcm, snd_pcm_channel_info_t *info)
{
    uint32_t  i;
    int chn_avail = 1;
 ...
 */
static int32_t imx_playback_aquire(HW_CONTEXT_T *imx, PCM_SUBCHN_CONTEXT_T **pc, ado_pcm_config_t *config,
                            ado_pcm_subchn_t *subchn, uint32_t *why_failed)
{
    ...
    if (imx->num_tx_aif == 1)
    {
        config->dmabuf.flags |= ADO_BUF_DMA_SAFE;
        imx->tx_fifo_watermark = calculate_fifo_watermark(config, &imx->tx_cfg);
    }

    if (ado_pcm_buf_alloc(imx->card, config, config->dmabuf.size, config->dmabuf.flags) == NULL) {
        ado_mutex_unlock(&imx->lock);
        return (errno);
    }
    ...
    if (imx->num_tx_aif == 1)
    {
        ...
        if (tinfo.src_addrs == NULL) {
            ado_pcm_buf_free(imx->card, config);
            ado_mutex_unlock(&imx->lock);
            return -1;
        }
 ...
 */
static int32_t imx_playback_release(HW_CONTEXT_T *imx, PCM_SUBCHN_CONTEXT_T *pc, ado_pcm_config_t *config)
{
    ado_mutex_lock(&imx->lock);
    /* Clear data */
    ado_pcm_buf_free(imx->card, config);
 ...
 */
static int32_t imx_capture_aquire(HW_CONTEXT_T *imx, PCM_SUBCHN_CONTEXT_T **pc, ado_pcm_config_t *config,
                           ado_pcm_subchn_t *subchn, uint32_t *why_failed)
{
    uint32_t i;
    imx_aif_t *aif = NULL;
 ...
 * Called by upper layer when client closes its connection to the device.
 */
static int32_t imx_capture_release(HW_CONTEXT_T *imx, PCM_SUBCHN_CONTEXT_T *pc, ado_pcm_config_t *config)
{
    ado_mutex_lock(&imx->lock);
    /* Clear data */
    ado_pcm_buf_free(imx->card, config);
 ...
 */
static int32_t imx_playback_trigger(HW_CONTEXT_T *imx, PCM_SUBCHN_CONTEXT_T *pc, uint32_t cmd)
{
    int32_t rtn = EOK;
 ...
 */
static int32_t imx_capture_trigger(HW_CONTEXT_T *imx, PCM_SUBCHN_CONTEXT_T *pc, uint32_t cmd)
{
    int rtn = EOK;
 ...
 */
static int query_hwi_device(HW_CONTEXT_T *imx, unsigned unit)
{
    uint32_t zero = 0;
...
#define NUM_RATES  (sizeof(ratelist) / sizeof(ratelist[0]))
    ...
    *hw_context = imx;
     ...
     */
    if (imx->num_tx_aif) {
        imx->tx_reg = ado_device_mmap(imx->card, imx->base, sizeof(imx_sai_rxtx_t));
    ...
    if (imx->num_rx_aif) {
        imx->rx_reg = ado_device_mmap(imx->card, imx->base + sizeof(imx_sai_rxtx_t), sizeof(imx_sai_rxtx_t));
    ...
    if (imx->num_tx_aif) {
        imx->aif[0].play_strm.pcm_caps.chn_flags = SND_PCM_CHNINFO_BLOCK
    ...
    if (imx->num_rx_aif) {
        ...
        imx->aif[0].cap_strm.pcm_caps.chn_flags = SND_PCM_CHNINFO_BLOCK
            | SND_PCM_CHNINFO_INTERLEAVE | SND_PCM_CHNINFO_BLOCK_TRANSFER | SND_PCM_CHNINFO_MMAP
            | SND_PCM_CHNINFO_MMAP_VALID;
        switch (imx->rx_cfg.sample_size) {
            case 2:
...
int ctrl_destroy(HW_CONTEXT_T *imx)
{
    ...
    if (imx->rx_reg) {
        ado_device_munmap(imx->card, imx->rx_reg, sizeof(imx_sai_rxtx_t));
    }
    if (imx->tx_reg) {
        ado_device_munmap(imx->card, imx->tx_reg, sizeof(imx_sai_rxtx_t));
    }
    if (imx->aif)
        ado_free(imx->aif);
    ado_free(imx);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/ctrl/mxsai/mxsai_dll.c $ $Rev: 906158 $")
#endif
