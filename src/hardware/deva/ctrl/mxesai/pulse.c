		...
		if (MsgReceivePulse (mps->chid, &pulse, sizeof (pulse), NULL) != -1)
		{
			switch (pulse.code)
			{
				...
				case PULSE_CODE_EXIT:
					ado_error_fmt("MX_ESAI: Got Exit pulse...");
					pthread_exit(NULL);
				default:
					break;
			}
			...
			ado_error_fmt("MX_ESAI: MsgReceivePulse() error %s",
						  strerror (errno));
	}
	...
	void (*handler) (HW_CONTEXT_T * hw_context, struct sigevent * event), HW_CONTEXT_T * hw_context)
{
	...
	{
		ado_error_fmt("MX_ESAI: unable to attach to pulse (%s)", strerror (err));
		return -err;
	}
...
my_detach_pulse (void **x)
{
	...
	ado_error_fmt("MX_ESAI: Deliver Exit pulse...");
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/ctrl/mxesai/pulse.c $ $Rev: 859532 $")
#endif
