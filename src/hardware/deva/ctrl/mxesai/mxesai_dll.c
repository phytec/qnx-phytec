	...
	ado_pcm_config_t * config, ado_pcm_subchn_t * subchn, uint32_t * why_failed)
{
	...
	if (ado_pcm_buf_alloc(mx->card, config, config->dmabuf.size, config->dmabuf.flags) == NULL)
	{
		ado_mutex_unlock(&mx->hw_lock);
		return (errno);
	}
	...
	if (mx->num_tx_aif == 1)
	{
		...
		if(NULL == tinfo.src_addrs)
		{
			...
			ado_pcm_buf_free(mx->card, config);
			ado_mutex_unlock (&mx->hw_lock);
			return -1;
		}
#if IMX_EDMA
...
void
ctrl_version (int *major, int *minor, const char **date)
{
...
ctrl_init (HW_CONTEXT_T ** hw_context, ado_card_t * card, char *args)
{
	...
	*hw_context = mx;
	 ...
	 */
	mx->esai = ado_device_mmap (mx->card, mx->esaibase, sizeof (esai_t));
...
ctrl_destroy (HW_CONTEXT_T * mx)
{
	...
	if (mx->esai)
	{
		/* Disable ESAI */
		mx->esai->ecr = 0x0;
		ado_device_munmap(mx->card, mx->esai, sizeof (esai_t));
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/ctrl/mxesai/mxesai_dll.c $ $Rev: 904201 $")
#endif
