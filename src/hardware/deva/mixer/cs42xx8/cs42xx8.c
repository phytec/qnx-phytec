 ...
 */
static int32_t cs42xx8_ainx_mute_control(MIXER_CONTEXT_T * cs42xx8, ado_mixer_delement_t * element,
                                         uint8_t set, uint32_t * val, void *instance_data)
{
    ...
    for (i = 0; i < rx_voices; i++)
    {
        if (set)
        {
            if (!(cs42xx8->ain_mute[idx] & (1<<i)) && (val[0] & (1<<i)))
            {
                altered = 1;
            ...
            else if ((cs42xx8->ain_mute[idx] & (1<<i)) && !(val[0] & (1<<i)))
            {
                altered = 1;
                                        ...
                                        uint8_t set, uint32_t * vol, void *instance_data)
{
    ...
    for (i = 0; i < rx_voices; i++)
    {
        if (set)
        {
            if (cs42xx8->ain_vol[i+offset] != vol[i])
            {
                altered = 1;
                cs42xx8->ain_vol[i+offset] = vol[i];
                /* Apply volume if not currently muted */
                if (!(cs42xx8->ain_mute[idx] & (1<<i))) {
                    cs42xx8_write(cs42xx8, ainx_vol_ctl + i, INPUT_VOL(cs42xx8->ain_vol[i+offset]), 0xFF);
                }
            }
        }
        ...
        {
            vol[i] = cs42xx8->ain_vol[i+offset];
        }
    }
                                          ...
                                          uint8_t set, uint32_t * val, void *instance_data)
{
    ...
    uint32_t hw_mute_mask = ADO_CONTIG_VOICE_MASK(tx_voices) << offset;

    if (set)
    {
        uint32_t hw_mute_map = (val[0] << offset) & hw_mute_mask;
        altered = (hw_mute_map != (cs42xx8->aout_mute & hw_mute_mask));
        if (altered)
        {
            cs42xx8->aout_mute &= ~hw_mute_mask;
    ...
    {
        val[0] = (cs42xx8->aout_mute & hw_mute_mask) >> offset;
    }
                                         ...
                                         uint8_t set, uint32_t * vol, void *instance_data)
{
    ...
    for (i = 0; i < tx_voices; i++)
    {
        if (set)
        {
            if (cs42xx8->aout_vol[i + offset] != vol[i])
            {
                altered = 1;
                cs42xx8->aout_vol[i + offset] = vol[i];
                /* Set volume */
        ...
        {
            vol[i] = cs42xx8->aout_vol[i + offset];
        }
    }
...
static int32_t cs42xx8_mixer_build(MIXER_CONTEXT_T * cs42xx8, ado_mixer_t * mixer)
{
    ...
    for (idx = 0; idx < cs42xx8->ndevs[SND_PCM_CHANNEL_PLAYBACK]; idx++)
    {
        ...
        if (!error && (elem = ado_mixer_element_volume1(mixer, "PCM Volume", chmap->channels,
        ...
        play_vol = elem;
        pre_elem = elem;
        if (!error && (elem = ado_mixer_element_sw1(mixer, "PCM Mute", chmap->channels,
        ...
        if (!error && (elem = ado_mixer_element_io(mixer, "PCM OUT", SND_MIXER_ETYPE_OUTPUT, 0, chmap)) == NULL) {
            error++;
        }
        if (!error && ado_mixer_element_route_add(mixer, pre_elem, elem) != 0) {
            error++;
        }
        if (!error && (play_grp = ado_mixer_playback_group_create(mixer, SND_MIXER_MASTER_OUT, chmap, play_vol, play_mute)) == NULL) {
            error++;
        }
    }
    ...
    for (idx = 0; idx < cs42xx8->ndevs[SND_PCM_CHANNEL_CAPTURE]; idx++)
    {
        ...
        name = (cs42xx8->params.rx_voices == 4) ? "PCM In Volume" : ((idx == 0) ? "Line In Volume" : "Mic In Volume");
        if (!error && (elem = ado_mixer_element_volume1(mixer, name,
                                                        chmap->channels, input_range,
        ...
        capture_vol = elem;
        pre_elem = elem;
        name = (cs42xx8->params.rx_voices == 4) ? "PCM In Mute" : ((idx == 0) ? "Line In Mute" : "Mic In Mute");
        ...
        capture_mute = elem;
        pre_elem = elem;
        name = (chmap->channels == 4) ? "PCMIn" : ((idx == 0) ? "LineIn" : "MicIn");
        if (!error && (elem = ado_mixer_element_io(mixer, name, SND_MIXER_ETYPE_INPUT, 0, chmap)) == NULL) {
            error++;
        }
        if (!error && ado_mixer_element_route_add(mixer, pre_elem, elem) != 0) {
            error++;
        }
        name = (chmap->channels == 4) ? SND_MIXER_PCM_IN : ((idx == 0) ? SND_MIXER_LINE_IN : SND_MIXER_MIC_IN);
        if (!error && (capture_grp = ado_mixer_capture_group_create(mixer, name, chmap, capture_vol,
                                                                    capture_mute, NULL, NULL)) == NULL) {
            error++;
        }
    }
    return (!error ? 0 : -1);
                ...
                void *raw_callbacks, int version)
{
    ...
    *context = cs42xx8;
    ...
    if (cs42xx8->params.get_chmap == NULL)
    {
        ado_error_fmt ("CS42XX8: get_chmap routine missing");
        ado_free(cs42xx8);
        return (-1);
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/deva/mixer/cs42xx8/cs42xx8.c $ $Rev: 903304 $")
#endif
