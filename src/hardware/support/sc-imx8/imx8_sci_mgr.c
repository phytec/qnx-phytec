...
int io_devctl(resmgr_context_t *ctp, io_devctl_t *msg, RESMGR_OCB_T *ocb)
{
    ...
    switch (msg->i.dcmd) {
        ...
        case IMX_DCMD_SC_MISC_GET_BOOT_TYPE:
            status=sc_misc_get_boot_type(sci->id, (sc_misc_bt_t *)&input_data->boot_type);
            nbytes = sizeof(sc_misc_bt_t);
            break;
        /* Calls sc_misc_get_boot_container() function */
        case IMX_DCMD_SC_MISC_GET_BOOT_CONTAINER:
            status=sc_misc_get_boot_container(sci->id, (uint8_t *)&input_data->boot_idx);
 ...
 * @retval sigevent
 */
const struct sigevent * mu_handler(void *area, int id)
{
    uint32_t reg;
 ...
 * @param data
 */
static void* imx_sc_drv_thread(void *data)
{
    struct _pulse pulse;
    imx_irq_event_t *cur_irq_event;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/support/sc-imx8/imx8_sci_mgr.c $ $Rev: 907901 $")
#endif
