...
int can_drvr_devctl(CANDEV *cdev, int dcmd, DCMD_DATA *data)
{
    ...
    switch (dcmd) {
        ...
        case CAN_DEVCTL_SET_MFILTER:
            ...
            if ((!(mbxid < devinfo->numrx)) || (devinfo->mode == CANDEV_MODE_RAW_FRAME)) {
                return (EINVAL);
            }
...
//            fprintf(stderr, "CANIMR%d = 0x%02x\n", mbxid, in32(devinfo->canlam + offset));
        ...
        case CAN_DEVCTL_SET_PRIO:
            ...
            if ((mbxid < devinfo->numrx) || (data->prio > FLEXCAN_MCF_TPL_MAXVAL)
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/can/mx7x/canmx7x.c $ $Rev: 893378 $")
#endif
