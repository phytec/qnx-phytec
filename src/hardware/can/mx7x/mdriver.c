...
// Function prototypes
void mdriver_print_data(CANDEV_FLEXCAN_INFO *devinfo);
 ...
 *
 * @return Matching interrupt vector or -1.
 */
int mdriver_init(CANDEV_FLEXCAN_INFO *devinfo, CANDEV_FLEXCAN_INIT *devinit)
{
    ...
    for (mindex = 0; mindex < num_mdrivers; mindex++) {
        ...
        if (mdriver[mindex].intr == (uint32_t)devinit->irqsys) {
#ifdef DEBUG_MDRVR
 ...
 *
 * @param devinfo Pointer to a device info structure.
 * @param canmid  CAN message ID.
 *
 * @return Matching mailbox ID or -1.
 */
static int mdriver_find_mbxid(CANDEV_FLEXCAN_INFO *devinfo, uint32_t canmid)
{
    unsigned int i;
 ...
 * @param devinit Pointer to a device init structure.
 */
void mdriver_init_data(CANDEV_FLEXCAN_INFO *devinfo, CANDEV_FLEXCAN_INIT *devinit)
{
    ...
    for (i = 0; i < mdata->nrx; i++) {
        ...
        if (mbxid != -1) {
            ...
            // No support for overwriting an old message since this
            // is called before we start processing new data
            if ((rxmsg = canmsg_dequeue_element(devlist[mbxid].cdev.free_queue)) != NULL) {
                ...
                rxmsg->cmsg.len = (mcanmsg[i].canmcf & IMX7_FLEXCAN_MSG_BUF_DLC_MASK) >> IMX7_FLEXCAN_MSG_BUF_DLC_SHIFT;
                ...
                rxmsg->cmsg.ext.is_extended_mid = (mcanmsg[i].canmcf & IMX7_FLEXCAN_MB_CNT_IDE) > 0;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/can/mx7x/mdriver.c $ $Rev: 893475 $")
#endif
