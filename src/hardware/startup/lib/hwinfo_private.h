...
* hwibus_add_pwm
* 
* Adds an 'hwinfo' entry for PWM devices.
* Added attribute tags are as follows...
* 
* 	- common attributes
* 	- input clock (1 per possible source)
...
* 			on error
*/
#define HWIATTR_PWM_T_INITIALIZER \
		{ \
			.common = HWIATTR_COMMON_INITIALIZER, \
			.num_clks = 0, .num_memaddr = 0, \
		}

#define HWIATTR_PWM_SET_NUM_CLK(__hwitag_p__, __num__) \
	do { \
		(__hwitag_p__)->num_clks = (__num__); \
	} while(0)

#define HWIATTR_PWM_SET_NUM_MEMADDR(__hwitag_p__, __num__) \
	do { \
		(__hwitag_p__)->num_memaddr = (__num__); \
	} while(0)

#define HWIATTR_PWM_SET_LOCATION(__hwitag_p__, ...)		HWIATTR_SET_LOCATION(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_PWM_SET_NUM_IRQ(__hwitag_p__, ...)		HWIATTR_SET_NUM_IRQ(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_PWM_SET_NUM_ERRATA(__hwitag_p__, ...)	HWIATTR_SET_NUM_ERRATA(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_PWM_SET_DLL(__hwitag_p__, ...)			HWIATTR_SET_DLL(&(__hwitag_p__)->common, __VA_ARGS__)
#define HWIATTR_PWM_SET_OPTSTR(__hwitag_p__, ...)		HWIATTR_SET_OPTSTR(&(__hwitag_p__)->common, __VA_ARGS__)

extern unsigned hwidev_add_pwm(const char *name, hwiattr_pwm_t *attr, unsigned bus_hwi_off);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/startup/lib/hwinfo_private.h $ $Rev: 909989 $")
#endif
