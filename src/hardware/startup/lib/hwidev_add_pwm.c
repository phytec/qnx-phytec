...
* 			on error
*/
unsigned hwidev_add_pwm(const char *device_name, hwiattr_pwm_t *attr, unsigned bus_hwi_off)
{
	/* we use the (unused) 'device.pnpid' as the device class identifier */
	unsigned hwi_off = hwidev_add(device_name, hwi_devclass_NONE, bus_hwi_off);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/startup/lib/hwidev_add_pwm.c $ $Rev: 909989 $")
#endif
