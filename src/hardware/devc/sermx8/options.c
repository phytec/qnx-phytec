 ...
 */
int query_hwi_device(TTYINIT_UART *dip, unsigned unit)
{
 ...
 * @return Execution status.
 */
int query_hwi_device_by_base_address(TTYINIT_UART *dip, unsigned dev_base)
{
...
unsigned options(int argc, char *argv[], uint8_t *smmu_ports_created)
{
    ...
    while (optind < argc) {
         ...
         * Options already used by io-char (do not use these!): b,e,E,f,F,s,S,C,I,O,o,v,m
         */
        while ((opt = getopt(argc, argv, IO_CHAR_SERIAL_OPTIONS "t:T:c:u:U:dmi:l:")) != -1) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devc/sermx8/options.c $ $Rev: 894855 $")
#endif
