...
unsigned long image_scan_secure (unsigned long start, unsigned long end) {
	...
	for (; start < end; start += 4) {
		...
		sha256_init(&shactx);
		sptr = (const unsigned char*)(start - startup_hdr.preboot_size);
		rem = startup_hdr.stored_size + startup_hdr.preboot_size;
		while (rem > 0) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/ipl/lib/image_scan_secure.c $ $Rev: 909395 $")
#endif
