...
int image_add_info(void *imaddr, struct startup_info_hdr *info)
{
	...
	if ((((unsigned long)sih - ram_addr) + info->size) > sizeof(struct startup_header)) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/ipl/lib/image_add_info.c $ $Rev: 823349 $")
#endif
