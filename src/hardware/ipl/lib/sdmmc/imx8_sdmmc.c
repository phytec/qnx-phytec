...
static void imx_set_frq(sdmmc_t *sdmmc, unsigned frq)
{
    ...
    out32(base + MX_MMCSYSCTL, (in32(base + MX_MMCSYSCTL) & ~
                                (MMCSYSCTL_SDCLKEN))); /* Clear SDCLKEN to change the SD clock frequency */

    out32(base + MX_MMCSYSCTL, in32(base + MX_MMCSYSCTL) & ~(MMCSYSCTL_SDCLKFSMASK |
                                                             MMCSYSCTL_DVSMASK)); /* Clear clock masks */
...
static void imx_set_bus_width(sdmmc_t *sdmmc, int width)
{
...
static void imx_set_bus_mode(sdmmc_t *sdmmc, int mode)
{
    return;
}

static void imx_set_timing(sdmmc_t *sdmmc, int timing)
{
...
static int imx_init_ctrl(sdmmc_t *sdmmc)
{
    unsigned base = sdmmc->hc->sdmmc_pbase;
    ...
    imx_set_frq(sdmmc, 400);

    out32(base + MX_MMCPROTOCTL, 0x8800020); /* Reset value for Protocol Control Register */

    out32(base + MX_MMCINTRSIGEN, 0x117F8033); /* Enable all interrupt signals */

    out32(base + MX_MMCINTRSTEN, 0x117F8033); /* Enable all interrupt status bits */

    out32(base + MX_MMCWATML, (0x80 << MMCWATML_RDWMLSHIFT)); /* Configure watermark value */

    return SDMMC_OK;
}

/* clean up the SDMMC controller */
int imx_sdmmc_fini(sdmmc_t *sdmmc)
{
    unsigned base = sdmmc->hc->sdmmc_pbase;
    ...
    out32(base + MX_MMCSYSCTL, MMCSYSCTL_IPP_RST_N);
...
static void imx_setup_dma_read(sdmmc_t *sdmmc)
{
    unsigned     base = sdmmc->hc->sdmmc_pbase;
    unsigned int sys_ctl;

    out32(base + MX_MMCBLATR, (sdmmc->cmd.bcnt << MMCBLATR_BLCNTSHIFT) | sdmmc->cmd.bsize);
    out32(base + MX_MMCDMASYSADD, (uint32_t)((long)sdmmc->cmd.dbuf));
...
static int imx_send_cmd(sdmmc_t *sdmmc)
{
    ...
    if (command->cmd == 0) {
        out32(base + MX_MMCSYSCTL, in32(base + MX_MMCSYSCTL) | MMCSYSCTL_INITA | (0x2 <<
                                                                                  MMCSYSCTL_DTOCVSHIFT)); /* Set Initialization Active */
    ...
    if (temp & MMCINTR_ERRI) {
        ...
        if (temp != 2) {
            ...
            if (sdmmc->card.state != MMC_IDLE) {
                ser_putstr("imx_send_cmd() failed at CMD");
                ser_putdec(command->cmd);
...
static int imx_pio_read(sdmmc_t *sdmmc, void *buf, unsigned len)
{
    unsigned      base = sdmmc->hc->sdmmc_pbase;
    sdmmc_cmd_t  *cmd  = &sdmmc->cmd;
    unsigned int  mix_ctrl = 0;
    ...
    /* setup PIO read */
    out32(base + MX_MMCBLATR, (1 << MMCBLATR_BLCNTSHIFT) | len);
    ...
    /* setup the argument register and send command */
    out32(base + MX_MMCCMDARG, cmd->arg);
    out32(base + MX_MMCCMDXTYPE, (cmd->cmd << MMCCMDXTYPE_CMDINXSHIFT) | MMCCMDXTYPE_CICEN | MMCCMDXTYPE_CCCEN |
          MMCCMDXTYPE_RSPTYPL48 | MMCCMDXTYPE_DPSEL);
    ...
    if (in32(base + MX_MMCINTRST) & MMCINTR_ERRI) {
        ...
        if (cmd->erintsts != 2) {
            if (sdmmc->card.state != MMC_IDLE) {
                ser_putstr("imx_pio_read() failed. MX_MMCINTRST: 0x");
...
static int imx_dma_mxblks(sdmmc_t *sdmmc)
{
    return (65535);
}
/*--------------------------------------*/
/* MX Host Controller               */
/*--------------------------------------*/
static sdmmc_hc_t imx_hc = {
    .set_clk = imx_set_frq,
    .set_bus_width = imx_set_bus_width,
    .set_bus_mode = imx_set_bus_mode,
    .set_timing = imx_set_timing,
    .send_cmd = imx_send_cmd,
    .pio_read = imx_pio_read,
    .init_hc = imx_init_ctrl,
    .dinit_hc = imx_sdmmc_fini,
    .signal_voltage = 0,
    .tuning = 0
};

void imx_sdmmc_init_hc(sdmmc_t *sdmmc, unsigned base, unsigned clock, int verbose)
{
    imx_hc.sdmmc_pbase = base;
    sdmmc->hc = &imx_hc;
    imx_hc.clock = clock;
    sdmmc->verbose = verbose;
    sdmmc->hc->max_blks = imx_dma_mxblks;
    sdmmc->caps = 0;
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/ipl/lib/sdmmc/imx8_sdmmc.c $ $Rev: 878192 $")
#endif
