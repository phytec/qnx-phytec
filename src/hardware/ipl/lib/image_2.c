...
#include "ipl.h"
    ...
    extern int checksum_aarch64(paddr_t addr, paddr_t len);
#endif
...
unsigned long image_scan_2 (unsigned long start, unsigned long end, int docksum) 
{
	...
	if (docksum) {
		...
		if (checksum_aarch64(start, startup_hdr.startup_size) != 0) {
#else
...
#elif defined(__aarch64__)
		if (checksum_aarch64(start + startup_hdr.startup_size, startup_hdr.stored_size - startup_hdr.startup_size) != 0) {
#else
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/ipl/lib/image_2.c $ $Rev: 840015 $")
#endif
