...
unsigned image_download_8250(unsigned baud, unsigned long port_address, unsigned long dst_address){
	...
	while(1){
		...
		if (record.cmd != DATA_CMD){
			...
			if (record.cmd == ECHO_CMD){
				put_byte(port_address,ECHO_CMD);
				continue;
			}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/ipl/lib/image_download_8250.c $ $Rev: 891287 $")
#endif
